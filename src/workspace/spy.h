...
void *get_shared_var(){
    int fd;
    fd = open(SYNC_FILE, O_CREAT|O_RDWR);
    if(fd == -1){
        printf("[ERROR] Sync file opened failed\n");
        exit(-1);
    }
    void * retVal = mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FILE, fd, 0);
    if(retVal == MAP_FAILED){
        printf("[ERROR] Mapping sync file failed\n");
        exit(-1);
    }
    return retVal;
}
